...
function normalizeData(){
  ...
  if(S.Member_now?.value){
    S.Member_now.value.forEach(m=>{
      if(m[MI.examTitle]!==undefined&&typeof m[MI.examTitle]!=='string')m[MI.examTitle]=String(m[MI.examTitle]);
      if(m[MI.school]!==undefined&&typeof m[MI.school]!=='string')m[MI.school]=String(m[MI.school]);
    });
  }
}
...
function applyField(row,type,field,rawVal){
  ...
  if(PIPE_FIELDS.has(field)&&fk!==undefined){
    ...
    row[fk]=isRet(type)?buildF4R(f):buildF4(f);
    return;
  }
  if(field==='partnerId'){row[fk]=setPipe(row[fk],9,rawVal);return;}
  if(field==='specialTag'){
  ...
  if(spec){const[idx,lo,hi]=spec;row[idx]=clampStr(rawVal,lo,hi);return;}
  if(type==='horse'||type==='inv'){row[+field]=rawVal;}
}
...
document.addEventListener('change',e=>{
  const t=e.target.dataset.t,i=e.target.dataset.i,f=e.target.dataset.f;
  if(!t||i===undefined||!f||!S)return;
  const src=DEL_KEY[t];
  if(src&&S[src]?.value[+i])applyField(S[src].value[+i],t,f,e.target.value);
});
...
function autoMaxGame(){
  ...
  if(S.FamilyData?.value){S.FamilyData.value[2]='80';S.FamilyData.value[3]='1600';}
  renderGameData();
}

function autoMax(type){
  if(!S)return;
  const STATS_100=['lit','mar','com','art','cunning','rep','health','stamina','mood','charm'];

  if(type==='member'&&S.Member_now){
    S.Member_now.value.forEach(m=>{
      STATS_100.forEach(k=>m[MI[k]]='100');
      m[MI.age]='21';m[MI.skillVal]='100';m[MI.examTitle]='9';m[MI.school]='3';
      const f=parseF4(m[MI.f4]);f.luck='100';f.talentVal='100';m[MI.f4]=buildF4(f);
    });
    renderMembers();
  }
  ...
  if(type==='spouse'&&S.Member_qu){
    S.Member_qu.value.forEach(s=>{
      ...
      s[SI.age]='21';s[SI.skillVal]='100';s[SI.relation]='100';
  ...
  if(type==='retainer'&&S.MenKe_Now){
    S.MenKe_Now.value.forEach(r=>{
      ...
      r[RI.age]='21';r[RI.charm]='100';r[RI.fee]='0';
      const f=parseF4(r[RI.nl]);f.luck='100';f.talentVal='100';r[RI.nl]=buildF4R(f);
    });
    renderRetainers();
  }
...
function addRetainer(){
  ...
  r[RI.age]='21';r[RI.health]='100';r[RI.stamina]='100';r[RI.mood]='50';
  [RI.rep,RI.charm,RI.lit,RI.mar,RI.com,RI.art,RI.cunning].forEach(i=>r[i]='50');
  r[RI.fee]='0';r[RI.specialTag]='';r[RI.pregnancy]='-1';
  S.MenKe_Now.value.push(r);renderRetainers();
}
...
function addAllItems(){
  S.Prop_have={__type:"System.Collections.Generic.List`1[[System.Collections.Generic.List`1[[System.String, mscorlib, Version=4.0.0.0, Culture=neutral, PublicKeyToken=b77a5c561934e089]], mscorlib, Version=4.0.0.0, Culture=neutral, PublicKeyToken=b77a5c561934e089]],mscorlib",value:[]};
  for(let id=0;id<=285;id++){S.Prop_have.value.push([String(id),'999999']);}
  renderInventory();
...
// ── Save ───────────────────────────────────────────────────────────
function doSave(){
  if(!S){setStatus('No save loaded',false);return;}
  flushGameData();
  normalizeData();
